...
if [ ! -x "$SSHPASS_PATH" ]; then
    echo "    'sshpass' not found, building...";
...
# from now on, should be able to do operations without sshpass - reboot and wait for it to come back
echo "Rebooting $RASPBERRY_PI...";
ssh $RASPBERRY_PI "sudo reboot now;"
HELLO="";
while [ "$HELLO" != "hello" ]; do
    echo ".";
    sleep 10;
    HELLO=$(ssh $RASPBERRY_PI "echo hello");
done;
...
# shutdown the box to complete everything
ssh $RASPBERRY_PI "sudo shutdown now";
